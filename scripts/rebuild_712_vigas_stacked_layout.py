#!/usr/bin/env python3
"""
Reconstrói a secção 7.1.2 (vigas protendidas / longarinas) com layout em coluna:

  «Para as cargas permanentes»
    → 7 figuras (momentos fletores, Tab. 1…7), cada uma com legenda + Fonte SCIA
    → 7 figuras (esforços cortantes permanentes, Tab. 1…7)

  «Para as cargas acidentais»
    → 7 figuras (momentos fletores, Tab. 1…7)
    → 7 figuras (esforço cortante, Tab. 1…7)

Total 28 figuras; prefixos de ficheiro iguais a rebuild_712_vigas.py (pasta WhatsApp do projeto).
Não repete o título «Diagramas de momentos e cortantes… por tabuleiro» antes de cada grupo
(evita a) b) c) repetidos).
"""
...
def _first_blip_snap(doc: Document, vigas_idx: int, trans_idx: int) -> tuple[ParaSnap, ParaSnap, ParaSnap, ParaSnap]:
    """Encontra primeiro bloco imagem+cap+fonte no intervalo para copiar formatação."""
    img_i = cap_i = font_i = None
    for i in range(vigas_idx + 1, trans_idx):
        if "blip" in doc.paragraphs[i]._p.xml:
            img_i = i
            break
    if img_i is None:
        raise RuntimeError("Nenhuma imagem no intervalo vigas para copiar estilo.")
    for j in range(img_i + 1, min(img_i + 8, trans_idx)):
        t = doc.paragraphs[j].text.strip()
        if t.startswith("Figura"):
            cap_i = j
            break
    if cap_i is None:
        raise RuntimeError("Legenda após imagem não encontrada.")
    for k in range(cap_i + 1, min(cap_i + 5, trans_idx)):
        t = doc.paragraphs[k].text.strip()
        if t.startswith("Fonte:"):
            font_i = k
            break
    if font_i is None:
        raise RuntimeError("Parágrafo Fonte não encontrado.")
    snap_empty = _snap(doc.paragraphs[img_i - 1]) if img_i > vigas_idx + 1 else _snap(doc.paragraphs[img_i])
    return _snap(doc.paragraphs[img_i]), _snap(doc.paragraphs[cap_i]), _snap(doc.paragraphs[font_i]), snap_empty


def rebuild():
    doc = Document(DOC_PATH)

    vigas_idx = trans_idx = None
    ...
    if vigas_idx is None or trans_idx is None:
        raise RuntimeError("Títulos vigas/transversinas não encontrados.")

    # numeração «Para as cargas…» (mesmo modelo que o documento já usa)
    sub_idx = None
    for i in range(vigas_idx + 1, trans_idx):
        if doc.paragraphs[i].text.strip() == "Para as cargas permanentes":
            sub_idx = i
            break
    if sub_idx is None:
        raise RuntimeError("Linha 'Para as cargas permanentes' não encontrada para extrair numPr.")
    num_pr_sub = _extract_num_pr(doc, sub_idx)

    snap_img, snap_cap, snap_fonte, snap_empty = _first_blip_snap(doc, vigas_idx, trans_idx)
    ...
    def add_list_line(text: str):
        ...
        _strip_paragraph_children(p)
        p.add_run(text)
        _set_list_num_pr(p, num_pr_sub)

    def add_figure_from_prefix(prefix: str, caption_body: str):
        nonlocal anchor
    ...
    # --- Cargas permanentes: 7 momentos + 7 cortantes, em coluna ---
    add_list_line("Para as cargas permanentes")
    add_blank()
    for tab in range(1, 8):
        pm, pc, _, _ = PREFIX_BY_TAB[tab]
        add_figure_from_prefix(
            pm,
            f"Diagramas de momentos fletores nas vigas protendidas do Tabuleiro {tab}.",
        )
    for tab in range(1, 8):
        _, pc, _, _ = PREFIX_BY_TAB[tab]
        add_figure_from_prefix(
            pc,
            f"Diagramas de esforços cortantes nas vigas protendidas do Tabuleiro {tab}.",
        )

    # --- Cargas acidentais: 7 momentos + 7 cortantes ---
    add_list_line("Para as cargas acidentais")
    add_blank()
    for tab in range(1, 8):
        _, _, am, _ = PREFIX_BY_TAB[tab]
        add_figure_from_prefix(
            am,
            f"Diagramas de momentos fletores nas vigas protendidas do Tabuleiro {tab}.",
        )
    for tab in range(1, 8):
        _, _, _, ac = PREFIX_BY_TAB[tab]
        add_figure_from_prefix(
            ac,
    ...
    doc.save(DOC_PATH)
    print("OK: secção 7.1.2 vigas em layout empilhado (28 figuras).")
